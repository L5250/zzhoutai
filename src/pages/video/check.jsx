...
export default class Check extends Component {
    ...
    videoData = (value, page) => {
        get('/zol-cms/seeker/video/page', { pageNo: page ? page : this.state.current, pageSize: 10, state: 1 }).then(res => {
            console.log(res)
            this.setState({
    ...
    // 通过
    apply=(record)=>{
        post('/zol-cms/seeker/video/pass',{seekerVideoId:record.videoId}).then(res=>{
            ...
            <Spin spinning={this.state.spinning}>
                ...
                >
                    ...
                    <Column title="操作" dataIndex="infoIntro"
                        render={(text, record) => (
                            <div>
                                <Button size="small" type="primary" onClick={()=>this.apply(record)}>通过</Button>
                                <Button size="small" type="danger"
                                    style={{ marginLeft: "10px" }}>拒绝</Button>
                            </div>
                        )}
